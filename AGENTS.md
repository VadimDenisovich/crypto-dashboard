...
> Отчёт: `.context/phase-6-ui-data-and-mvp-hardening.md`.

1. **Dashboard-виджеты на живых данных:** `BalanceWidget` → `/api/balances/summary`,
   `PositionsWidget` → `/api/positions`, `ChartWidget` → `/api/candles/{exchange}/{symbol}/{timeframe}`
   (recharts + выбор символа/ТФ). Все polling 15с.
2. **Новые backend-эндпоинты:** `GET /api/positions`, `GET /api/balances/summary`,
   `GET /api/candles/{exchange}/{symbol}/{timeframe}`.
3. **Command listener fix:** `COMMAND_UPDATE` теперь передаёт payload в orchestrator.
4. **Event projector:** добавлен обработчик `engine.positions_update` → БД + WS.
5. **Stop-loss / take-profit:** `PositionManager` проверяет уровни на каждом candle,
   авто-закрытие через тот же RiskManager → OrderExecutor. Параметры `stop_loss_pct` /
   `take_profit_pct` в `bot.params` и `signal.metadata`.
6. **Сверка позиций при старте:** `orchestrator.start_strategy` вызывает
   `adapter.get_positions()`, фильтрует по символу, передаёт в `PositionManager.reconcile()`.
7. **Миграция `positions_snapshots`:** `0006` + импорт в `alembic/env.py`.
8. **Healthcheck engine:** реальный ping Redis вместо `import sys; sys.exit(0)`.
9. **Бэктест fix:** pandas/pyarrow установлены, `save_parquet` падает в CSV при отсутствии,
   логи в stderr, чистый JSON в stdout.
10. **Frontend-тесты:** 88 тестов, 90.5% lines coverage.
...
- Docker-образы используют `python:3.14-slim` (пре-релиз, заменить на 3.12).
